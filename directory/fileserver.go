...
import (
	...
	"github.com/khulnasoft-lab/kengine_utils/telemetry"
...
var (
	FileServerExternal       = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_EXTERNAL", "false") == "true"
	FileServerProtocol       string
	FileServerSecure         = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_SECURE", "false") == "true"
	FileServerHost           = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_HOST", "kengine-file-server")
	FileServerPort           = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_PORT", "9000")
	FileServerRegion         = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_REGION", "")
	FileServerBucket         = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_BUCKET", string(NonSaaSDirKey))
	FileServerDatabaseBucket = utils.GetEnvOrDefault("KENGINE_FILE_SERVER_DB_BUCKET", string(DatabaseDirKey))
...
func (mfm *FileServerFileManager) ExposeFile(ctx context.Context, filePath string, addFilePathPrefix bool, expires time.Duration, reqParams url.Values, consoleURL string) (string, error) {
	...
	if !FileServerExternal {
		...
		// Format: kengine.customer.com:8080 or 56.56.56.56
		if consoleURL == "" {
